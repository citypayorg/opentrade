...
$(() => {
    ...
    UpdateCTPFromLB();
    setInterval(UpdateCTPFromLB, 30000);
});

function UpdateCTPFromLB()
{
  ...
  const CTP_price = storage.getItem("CTP_BTC_Price");
  const g_CTP_BTC_Price = (CTP_price == null || !CTP_price.value) ? 1000000 : CTP_price.value;
  ...
  fetch('/bitcoinaverage/ticker-all-currencies/')
    ...
    function UpdateCTPInfo() {
      ...
      const USD = g_LB_Data.USD.rates.last/(g_CTP_BTC_Price+1);
      ...
      if (CTP != 'BTC')
      {
        $('#id_CTP_info').append($('<li class="breadcrumb-item">1 ' + CTP + ' = '+BTC.toFixed(8)+' BTC</li>'));
        $('#id_CTP_info').append($('<li class="breadcrumb-item">'+USD.toFixed(3)+' USD</li>'));
        $('#id_CTP_info').append($('<li class="breadcrumb-item">'+EUR.toFixed(3)+' EUR</li>'));
      }
      else
      {
        $('#id_CTP_info').append($('<li class="breadcrumb-item">1 ' + CTP + ' = '+USD.toFixed(2)+' USD</li>'));
        $('#id_CTP_info').append($('<li class="breadcrumb-item">'+EUR.toFixed(2)+' EUR</li>'));
      }
...
function SetChartLegend()
{
  ...
  const CTP = coinNameToTicker[utils.MAIN_COIN].ticker; 
  const COIN = coinNameToTicker[g_CurrentPair].ticker
  

  $.getJSON( "/api/v1/public/getmarketsummary?market="+CTP+"-"+COIN+"&period="+g_currentChartPeriod, ret => {
    if (!ret || !ret.success || ret.success != true || CTP != coinNameToTicker[utils.MAIN_COIN].ticker || COIN != coinNameToTicker[g_CurrentPair].ticker) 
      return;
    ...
    const legend = $(
      '<ul class="nav" style="line-height: 30px;">'+
        ...
        '<li class="nav-item mr-3"><h4>'+COIN+' / '+CTP+'</h4></li>'+
