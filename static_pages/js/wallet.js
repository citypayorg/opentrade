...
function UpdateWallet(data)
{
    ...
    const CTP = coinNameToTicker[utils.MAIN_COIN] ? coinNameToTicker[utils.MAIN_COIN].ticker || 'BTC' : 'BTC';
    ...
    let tdCoin = '';
     if (data.coin.ticker == CTP) {
        tdCoin = $('<td scope="col" class="align-middle"> '+icon+unescape(data.coin.name)+'</td>');
    } else {
	    tdCoin = $('<td scope="col" class="align-middle"> <a href="/market/'+CTP+'-'+data.coin.ticker+'">'+icon+unescape(data.coin.name)+'</a></td>');
    }
